...
cc -Wall -Wextra -Werror -o ft_str_is_numeric ft_str_is_numeric.c main.c
if [ $? -ne 0 ]; then
...
longest_test="$> ./ft_str_is_numeric \"123Numbers\""
...
run_test() {
	...
	echo "$separator"
	echo "$> ./ft_str_is_numeric \"$input\""

	output=$(./ft_str_is_numeric "$input")
...
run_test "123456" 1 || all_tests_passed=false
run_test "123 456" 0 || all_tests_passed=false
run_test "" 1 || all_tests_passed=false
run_test "123Numbers" 0 || all_tests_passed=false
run_test "123!@#" 0 || all_tests_passed=false
