...
cc -Wall -Wextra -Werror -o ft_strcpy ft_strcpy.c main.c
if [ $? -ne 0 ]; then
...
longest_test="$> ./ft_strcpy \"Hello, World!\" | cat -e"
...
run_test() {
	...
	echo "$separator"
	echo "$> ./ft_strcpy \"$input\" | cat -e"

	output=$(./ft_strcpy "$input" | cat -e)
...
run_test "Test string" || all_tests_passed=false
run_test "" || all_tests_passed=false
run_test "Another test" || all_tests_passed=false
