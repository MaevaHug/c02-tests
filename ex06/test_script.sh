...
cc -Wall -Wextra -Werror -o ft_str_is_printable ft_str_is_printable.c main.c
if [ $? -ne 0 ]; then
...
longest_test="$> ./ft_str_is_printable \"$'Hello\tWorld'\""
...
run_test() {
	...
	echo "$separator"
	printf "$> ./ft_str_is_printable \"%q\"\n" "$input"

	output=$(./ft_str_is_printable "$input")
...
run_test "HelloWorld" 1 || all_tests_passed=false
run_test "Hello World" 1 || all_tests_passed=false
run_test "" 1 || all_tests_passed=false
run_test $'Hello\nWorld' 0 || all_tests_passed=false
run_test $'Hello\tWorld' 0 || all_tests_passed=false
