...
cc -Wall -Wextra -Werror -o ft_str_is_uppercase ft_str_is_uppercase.c main.c
if [ $? -ne 0 ]; then
...
longest_test="$> ./ft_str_is_uppercase \"ABC DEF\""
...
run_test() {
	...
	echo "$separator"
	echo "$> ./ft_str_is_uppercase \"$input\""

	output=$(./ft_str_is_uppercase "$input")
...
run_test "ABCDEF" 1 || all_tests_passed=false
run_test "ABC DEF" 0 || all_tests_passed=false
run_test "" 1 || all_tests_passed=false
run_test "ABCdef" 0 || all_tests_passed=false
