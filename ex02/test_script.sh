...
cc -Wall -Wextra -Werror -o ft_str_is_alpha ft_str_is_alpha.c main.c
if [ $? -ne 0 ]; then
...
longest_test="$> ./ft_str_is_alpha \"With_Symbols!\""
...
run_test() {
	...
	echo "$separator"
	echo "$> ./ft_str_is_alpha \"$input\""

	output=$(./ft_str_is_alpha "$input")
...
run_test "123Numbers" 0 || all_tests_passed=false
run_test "With_Symbols!" 0 || all_tests_passed=false
